...
class _PickupDropViewState extends State<PickupDropView> {
  ...
  Future<void> _calculateDistance() async {
    ...
    try {
      ...
      debugPrint(
        "Distance between pickup and drop: ${distance.toStringAsFixed(2)} km",
      );
    } catch (e) {
  ...
  Widget _swapButton() {
    return GestureDetector(
      onTap: () {
        ...
        context.read<LocationBloc>().add(SetPickupLocation(drop));

        context.read<LocationBloc>().add(SetDropLocation(pickup));
      },
      child: Container(
        height: 42,
        width: 42,
      ...
      child: SizedBox(
        ...
        child: ElevatedButton(
          style: ElevatedButton.styleFrom(
            backgroundColor: isEnabled ? Colors.orange : Colors.grey.shade400,
          ...
          onPressed: isEnabled
              ? () async {
                  try {
                    final pickupLatLng = await locationService
                        .getLatLngFromAddress(pickupController.text);

                    final dropLatLng = await locationService
                        .getLatLngFromAddress(dropController.text);
